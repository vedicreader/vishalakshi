...
# %% auto #0
__all__ = ['VAULT_SP', 'dflt_model', 'pii_model_', 'LOCAL_RUNTIMES', 'PII_SP', 'CHAT', 'CHAT_CACHE', 'new_chat', 'is_stock_chat',
           'use_chat', 'mk_prompt', 'split_reasoning', 'cited', 'doc_note', 'CachedChat']
...
#: The model that answers when the retrieved sections turn out to be somebody's business.
#: A separate setting from `$VISHALAKSHI_MODEL` because the whole point is that it is a
#: different model: this one has to be on this machine.
pii_model_ = os.getenv('VISHALAKSHI_PII_MODEL') or dflt_model

#: The runtimes that run on this machine. `remote` is the one that does not, and it is the
#: one thing the gate is protecting against, so the list is written as what is allowed rather
#: than as what is refused -- a runtime nobody here has heard of is not assumed to be local.
...
def ask(self:Vault,
        ...
        pii:str='local',       # what to do when the sections hold personal information
    ...
    `pii_ctx` reports on the sections retrieval actually chose -- not on the vault, and not on
    the document, because a vault holding one medical letter among four hundred papers is only
    a private question when the letter is in the room. What happens then is `pii`:

    - `'local'` (the default) answers on `pii_model`, under `PII_SP`, and **verifies the chat it
      was handed is a local runtime before sending a single character**. A caller that lends a
      hosted chat gets a refusal, not a leak: the check is on the object, after it is built, so
      it does not depend on the caller having been honest about the name.
    - `'redact'` masks every match and answers normally, wherever `model` points. Cheaper, and
      it loses the ability to answer anything that turns on the details.
    - `'refuse'` returns the finding and no answer at all.
    - `'off'` does not look.

    `instruction` is the second half of the two-turn shape `PII_SP` asks for: the local model
    says what it would need to be told, the questioner sends it, and it arrives here.
    """
    ...
    if private and pii == 'local':
        # A different model, a different briefing, and a check that the thing we were handed
        # really does run here. `sp` is replaced rather than appended to: the vault prompt tells
        # it to cite every claim, and a citation is a pointer at the text it must not repeat.
    ...
    if private and pii != 'redact':
        ...
        if pii == 'refuse' or rt not in LOCAL_RUNTIMES:
            # The refusal is the feature. Everything else here is a policy the caller chose;
            # this is the one case where doing what was asked would be the bug.
    ...
    if schema is not None:
        ...
        if private:
            # The prompt asked it not to, and a small model asked not to do something is not the
            # same as a small model that did not. This is the arithmetic saying so, on the way
            # out, where a slip costs a masked token instead of somebody's account number.
            leaked = pii_ctx(AttrDict(results=[AttrDict(text=out.answer)], related=[]))
            if leaked.has_pii:
                out.answer, out.leaked = redact(out.answer), leaked.identifying

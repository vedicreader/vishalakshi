...
# %% auto #0
__all__ = ['VAULT_SP', 'MODELS', 'dflt_model', 'resolve_model', 'model_cached', 'mk_prompt', 'split_reasoning', 'cited']
...
def model_cached(mid:str) -> bool:
    ...
    What separates "use a model when the cheap path cannot decide" from "start a two gigabyte
    download because a one-line note was ambiguous". The weights are either on this machine or they
    are not, and asking is free."""
    try:
...
def ask(self:Vault,
    ...
    A cited section whose `node_id` is `None` came from the code leg — the citation is a `path:line`
    on disk rather than something `read()` can open. `context.code` counts how many arrived."""
    from rishi.core import resp_text
    ctx = self.context(question, sections=sections, related=related, kind=kind, code=code, dir=dir, **kw)

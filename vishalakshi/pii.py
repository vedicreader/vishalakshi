...
# %% auto #0
__all__ = ['MAX_SCAN', 'DENSE', 'PATTERNS', 'IDENTIFYING', 'luhn', 'name_spans', 'pii_spans', 'pii_report', 'redact',
           'redact_obj', 'pii_ctx']
...
#: `name` is only ever counted when a caller asks for it: see `pii_spans(ner=True)`.
IDENTIFYING = frozenset({'email', 'card', 'iban', 'ssn', 'nhs', 'phone', 'dob', 'passport', 'licence', 'account',
                         'sortcode', 'medical', 'secret', 'name'})
...
def name_spans(text:str,     # what to scan
               mx:int=None,  # chars handed to the extractor; None -> `extract.NER_CHARS`
) -> L:
    """Personal names, from the honorific-anchored extractor `signals` already uses.
    ...
    for surface, label in _noun_ents(text):
        ...
        out += [(m.start(), m.end(), 'name', surface) for m in re.finditer(re.escape(surface), text)]
    return L(out)
...
def pii_spans(text:str,          # what to scan
              ...
              ner:bool=False,    # also look for names, which costs an entity pass over `NER_CHARS`
) -> L:
    """Every match, as `(start, end, kind, text)`, longest first and never overlapping."""
    text = _scan_text(text, mx)
    want = set(kinds or (*_COMPILED, 'name'))
    found = []
    ...
    if ner and 'name' in want: found += list(name_spans(text))
...
def pii_report(text:str,          # what to scan
    ...
    `scanned_ner` is in the report because no `name` count means nothing on its own: names are not looked for unless asked.
    """
    ...
    return AttrDict(has_pii=bool(ident), kinds=counts, identifying=ident, n=len(spans), scanned=n,
                    scanned_ner=bool(ner), density=round(1000 * len(spans) / max(n, 1), 3), spans=spans)

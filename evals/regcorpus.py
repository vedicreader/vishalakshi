"""Real regulatory documents, for measuring a detector against prose nobody wrote as a test case.

The generated corpus in `evals/pii.py` can be resampled and has ground truth. That is what makes it
useful and what makes it blind: every lookalike in it is one somebody thought of. Legislation is the
opposite. It is public, it is dense with numbers that are not identity, and it holds no real
person's details, so all of it is a labelled negative nobody had to annotate.

Five jurisdictions. A detector written against one is a detector tuned to its digit shapes.

- **EU**: eight acts as PDFs from `litesearch/examples/pdfs`, read through `pdf_parse` so the text
  keeps the line breaks and header noise a real ingest keeps, plus the AI Act and the GDPR.
- **US**: 45 CFR Part 164. Its citations (`42 U.S.C. 1302(a)`, `Pub. L. 104-191`, `65 FR 82802`)
  are different digit shapes from EU numbering.
- **Australia**: the Privacy Act 1988, and the Telecommunications Act 1997, a statute about
  telephone numbering.
- **India**: the Penal Code, Criminal Procedure Code, Evidence Act and DPDP Act 2023. Indian digit
  grouping is 2-2-3 (`12,34,567`), and amounts run in lakhs.
- **Thailand**: the PDPA (B.E. 2562), in Thai script and numerals. No pattern here can read it.

Southeast Asia is one statute rather than four. Singapore Statutes Online, AGC Malaysia and the
Philippine Official Gazette are outside what the sandbox reaches, and no GitHub mirror of their
English text turned up.

    python -m evals.regcorpus          # fetch, extract, report sizes
"""
...
#: name -> (url, how to read the bytes). Mirrors throughout: eur-lex.europa.eu, huggingface.co,
#: legislation.gov.au, indiacode.nic.in and the US hosts are all outside what the sandbox reaches.
#: Each is a copy of a public act, not the register.
SOURCES = {
    'eu_ai_act_2024_1689': (
...
#: Statutes published one file per section. Cloned, not fetched: codeload tarballs 403 through the
#: sandbox proxy, and ninety raw requests is a crawl.
CLONES = {
...
def _get(url, dest, tries=4):
    "Download with backoff. A partial file is removed, not left to poison a later run."
...
def untag(s:str) -> str:
    "Tags out, entities in, spaces collapsed. Enough for a detector, not for a reader."
...
def india_text(d) -> str:
    "Section titles and bodies from the Indian-Law-Penal-Code-Json section list."
    return '\n\n'.join(f"{r.get('section_title') or ''}\n{r.get('section_desc') or ''}".strip()
                       for r in d if isinstance(r, dict))


def gdpr_text(d) -> str:
    "Titles and paragraph text from GDPRtEXT's nested chapter/article/point JSON."
    out = []
    def walk(o):
...
def clone():
    "Shallow-clone the one-file-per-section statutes. Returns name -> directory."
    out = {}
...
def _pdf_text(p:Path) -> str:
    "Pages joined, cached under the PDF's name so a re-run does not re-parse."
